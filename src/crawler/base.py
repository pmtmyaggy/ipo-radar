...
class BaseCrawler(ABC):
    ...
    def _request(
        ...
        **kwargs
        ...
        self._logger.debug(
            f"{method} {url} - {response.status_code} - {duration:.0f}ms"
        )
    ...
    def _check_exists(
        ...
        """检查记录是否已存在."""
        query = session.query(model_class) if session else None
        
        if query is None:
            with self.db_manager.session_scope() as s:
                return s.query(model_class).filter(
                    getattr(model_class, key) == value
                ).first() is not None
        else:
            return query.filter(getattr(model_class, key) == value).first() is not None
    ...
    def _log_crawl(
        ...
        try:
            ...
            log = CrawlLogModel(
                ...
                started_at=datetime.fromtimestamp(self._start_time) if self._start_time else datetime.now(),
