...
from src.radar.monitor import IPORadar, WatchlistManager
    ...
    with st.sidebar:
        ...
        if st.button("🔄 立即扫描", use_container_width=True):
            with st.spinner("扫描中..."):
                ...
                st.success(f"扫描完成！发现 {result.strong_opportunity_count} 个强烈机会")
...
def render_overview():
    ...
    with col4:
        st.metric(
            label="⏰ 即将到期禁售期",
            value=sum(1 for r in result.reports 
                     if r.get('windows', {}).get('lockup_days_until', 999) <= 14),
        )
    ...
    if result.reports:
        ...
        for report in result.reports:
            df_data.append({
                "股票": report['ticker'],
                "公司": report.get('company_name', '-')[:20],
                "上市天数": report.get('days_since_ipo', '-'),
                "vs IPO": f"{((report.get('price_vs_ipo', 1) - 1) * 100):+.1f}%" 
                         if report.get('price_vs_ipo') else '-',
                "活跃窗口": get_active_window_text(report),
        ...
        selected = st.selectbox(
            ...
            format_func=lambda x: f"{x} - {next((r.get('company_name', '') for r in result.reports if r['ticker'] == x), '')[:30]}"
        )
...
# 个股详情
def render_stock_detail(ticker: str = None):
    """渲染个股详情."""
    if ticker is None:
...
def create_candlestick_chart(bars, report):
    """创建K线图."""
    import pandas as pd
