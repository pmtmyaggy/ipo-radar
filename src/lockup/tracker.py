...
class LockupTracker:
            ...
            with self.db_manager.session_scope() as session:
                ...
                if result:
                    return LockupInfo(
                        ...
                        locked_holders=result.locked_holders or [],
                        ...
                        supply_impact_pct=result.supply_impact_pct,
    ...
    def get_upcoming_expiries(
        ...
        if self.db_manager:
            ...
            with self.db_manager.session_scope() as session:
                results = session.query(LockupInfoModel).filter(
                ...
                for result in results:
                    ...
                    # 过滤低影响
                    if info.supply_impact_pct and info.supply_impact_pct >= min_impact_pct:
                        events.append(info)
    ...
    def estimate_price_impact(self, supply_impact_pct: float, holder_types: list) -> str:
        """预估价格影响."""
        if supply_impact_pct > self.HIGH_IMPACT_PCT or \
           any(h in holder_types for h in ["vc", "pe"]):
            return "high"
