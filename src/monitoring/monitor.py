...
class MonitorMetrics:
    ...
    def get_unhealthy_crawlers(self) -> List[str]:
        """获取不健康的爬虫列表."""
        return [
            name for name, metrics in self.crawler_metrics.items()
    ...
    def get_stale_tables(self) -> List[str]:
        """获取过时的数据表列表."""
        return [
            name for name, metrics in self.database_metrics.items()
            if metrics.is_stale
        ]
...
class CrawlerMonitor:
    ...
    def generate_report(self) -> str:
        ...
        for name, db_metrics in metrics.database_metrics.items():
            ...
            update_time = db_metrics.last_update_time.strftime('%H:%M') if db_metrics.last_update_time else 'Never'
            lines.append(
                f"{status} {name}: "
                f"记录数={db_metrics.record_count}, "
                f"最后更新={update_time}"
            )
...
# 装饰器：自动记录爬虫请求指标
def monitored(crawler_name: str):
    """监控装饰器.
