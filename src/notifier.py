...
"""
    ...
    def send_daily_summary(self, result: ScanResult) -> bool:
        """发送每日扫描摘要.
...
class NotificationManager:
    ...
    def process_scan_result(self, result: ScanResult) -> None:
        ...
        for report in result.reports:
            if report.get("overall_signal") == "STRONG_OPPORTUNITY":
                self.notifier.send_strong_opportunity_alert(dict(report))

            # 检查禁售期紧急告警
            windows = report.get("windows", {})
            days = windows.get("lockup_days_until")
            if days is not None:
                if days <= 3:
                    self.notifier.send_lockup_warning(
                        report["ticker"],
                        days,
                        float(windows.get("supply_impact_pct") or 0),
                    )
