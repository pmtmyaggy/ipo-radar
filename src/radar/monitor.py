...
from src.crawler.models.schemas import IPOEvent, IPOStatus
...
class IPOTrackingStatus:
    ...
    price_vs_ipo: Optional[float] = None  # 当前价 / 发行价
...
class IPORadar:
    ...
    AUTO_REMOVE_DAYS = 365         # 上市365天后自动移除
    ...
    def _db_to_tracking_status(self, db_event) -> IPOTrackingStatus:
        """将数据库事件转换为跟踪状态."""
        today = date.today()
    ...
    def add_to_watchlist(self, ticker: str, ipo_date: Optional[date] = None) -> bool:
        """添加股票到观察名单.
            ...
            if self.db_manager:
                ...
                with self.db_manager.session_scope() as session:
                    db_event = IPOEventModel(
                        ...
                        ipo_date=ipo_date or date.today(),
                        status="trading",
                    )
    ...
    def _get_stock_info(self, ticker: str, ipo_date: Optional[date]) -> dict:
        """获取股票信息."""
        info = {"ticker": ticker}
    ...
    def update_status(self, ticker: str) -> Optional[IPOTrackingStatus]:
        """更新指定IPO的状态.
    ...
    def get_active_tickers(self) -> list[str]:
        ...
        active = [
            ...
            if s.status in ["upcoming", "active"]
        ]
...
class WatchlistManager:
    ...
    def bulk_add(self, tickers: list[str]) -> dict:
        """批量添加.
    ...
    def bulk_remove(self, tickers: list[str]) -> dict:
        """批量移除."""
        results = {"success": [], "failed": []}
