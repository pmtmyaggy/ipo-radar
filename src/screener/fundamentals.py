...
class ScreenCriteria:
    ...
    min_cash_runway_months: float | None = None
    max_debt_to_assets: float | None = None
    min_market_cap: float | None = None
    required_underwriters: list | None = None
...
class FundamentalScreener:
    ...
    def screen(self, tickers: list[str], criteria: ScreenCriteria | None = None) -> list[str]:
        """根据基本面条件筛选股票.
    ...
    def _get_s1_metrics(self, ticker: str) -> S1Metrics | None:
        """获取S-1关键指标."""
        # 尝试从数据库获取
        if self.db_manager:
            from src.crawler.models.database import IPOEventModel, S1FilingModel

            with self.db_manager.session_scope() as session:
                # 查找最新的S-1
                result = (
                    session.query(S1FilingModel)
                    .filter(
                        S1FilingModel.cik.in_(
                            session.query(IPOEventModel.cik).filter_by(ticker=ticker)
                        )
                    )
                    .order_by(S1FilingModel.filed_date.desc())
                    .first()
                )

                if result:
                    revenue_growth = self._to_decimal(result.revenue_yoy_growth)
                    gross_margin = self._to_decimal(result.gross_margin)
                    return S1Metrics(
    ...
    def _calculate_cash_runway(self, s1_filing: Any) -> Decimal | None:
        """计算现金跑道（月）."""
        ...
        if s1_filing.net_income < 0:
            ...
            if monthly_burn > 0:
                return self._to_decimal(s1_filing.cash_and_equivalents / monthly_burn)

        return None

    def _calculate_debt_ratio(self, s1_filing: Any) -> Decimal | None:
        """计算债务/资产比."""
        ...
        if estimated_assets > 0:
            return self._to_decimal(s1_filing.total_debt / estimated_assets)

        return None

    def _to_decimal(self, value: Any) -> Decimal | None:
        """将数据库/第三方返回值安全转换为 Decimal."""
        if value is None:
            return None
